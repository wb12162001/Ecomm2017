//------------------------------------------------------------------------------
// <copyright file="SYS_CONFIGController.cs">
//		开发：Ben Wang  
...
namespace Ecomm.Site.WebApp.Areas.SysConfig.Controllers
{
    ...
    public class SYS_CONFIGController : AdminController
	{
		//
		// GET: /Authen/User/
		...
		#region 属性
		[Import]
		public ISYS_CONFIGService SYS_CONFIGService { get; set; }
		#endregion	
        ...
        public ActionResult List(DataTableParameter param)
        {
			...
			int total = SYS_CONFIGService.SYS_CONFIGList.Count(expr);
			
			var filterResult = SYS_CONFIGService.SYS_CONFIGList.Where(expr).Select(t => new SYS_CONFIGModel
											 {
                                                 ID = t.ID,
                                                 ConfigName = t.ConfigName,
                                                 ConfigContent = t.ConfigContent,
                                                 FieldProperty = t.FieldProperty,
                                                 IsSystem = t.IsSystem,
											 }
										  ).OrderBy(t=>t.ConfigName).Skip(param.iDisplayStart).Take(param.iDisplayLength).ToList();
		...
		/// </summary>
		/// <returns></returns>
		private Expression<Func<SYS_CONFIG, Boolean>> BuildSearchCriteria()
		{
			DynamicLambda<SYS_CONFIG> bulider = new DynamicLambda<SYS_CONFIG>();
			Expression<Func<SYS_CONFIG, Boolean>> expr = null;
            /*
            if (!string.IsNullOrEmpty(Request["ProductName"]))
            {
                var data = Request["ProductName"].Trim();
                Expression<Func<SYS_CONFIG, Boolean>> tmp = t => t.ProductName.Contains(data);
                expr = bulider.BuildQueryAnd(expr, tmp);
            }
            if (!string.IsNullOrEmpty(Request["Description"]))
            {
                var data = Request["Description"].Trim();
                Expression<Func<SYS_CONFIG, Boolean>> tmp = t => t.Description.Contains(data);
                expr = bulider.BuildQueryAnd(expr, tmp);
            }
            if (Request["Enabled"] == "0" || Request["Enabled"] == "1")
            {
                var data = Request["Enabled"] == "1" ? 1 : 0;
                Expression<Func<SYS_CONFIG, Boolean>> tmp = t => t.Status == data;
                expr = bulider.BuildQueryAnd(expr, tmp);
            }
            ...
            if (!string.IsNullOrEmpty(Request["StartTime"]))
            {
                ...
                Expression<Func<SYS_CONFIG, Boolean>> tmp = t => t.LeadTime >= data;
                expr = bulider.BuildQueryAnd(expr, tmp);
            }
            ...
            */
            Expression<Func<SYS_CONFIG, Boolean>> tmpSolid = t => 1 == 1;
        ...
        public ActionResult Delete(string  ID)
        {
			...
			this.UpdateBaseData<SYS_CONFIGModel>(model);
            */
			OperationResult result = SYS_CONFIGService.Delete(ID);
            return Json(result);
        }
	}
}
